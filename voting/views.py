...
def submit_ballot(request):
    ...
    for position in positions:
        ...
        try:
            ...
            existing_vote = Votes.objects.filter(voter=voter, position=position).first()
            if existing_vote:
                messages.error(
                    request, "You have already voted for " + position.name)
                return redirect(reverse('show_ballot'))
            vote = Votes()
            ...
            vote.save()
        except Votes.MultipleObjectsReturned:
            messages.error(
                request, "Multiple votes detected for " + position.name + ". Please contact administrator.")
            return redirect(reverse('show_ballot'))
        except Exception as e:
